...
public class LocalRoleServiceImpl implements RoleService, EventListener<EventCustomizingUpdate> {

    private static final Logger LOG = LoggerFactory.getLogger(LocalRoleServiceImpl.class);
    ...
    protected void activate(ComponentContext context) {
        ...
        invalidatePermits();
        LOG.info(MessageFormat.format("[RoleService][type=local] {0} : activated",
    ...
    protected void deactivate(ComponentContext context) {
        invalidatePermits();
        this.context = null;
        LOG.info(MessageFormat.format("[RoleService][type=local] {0} : deactivated",
                (String) context.getProperties().get(ComponentConstants.COMPONENT_NAME)));
    }
